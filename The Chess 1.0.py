...
class Battlefield():
    def __init__(self):
        ...
        self.field[0] = [Rook(Black), Knight(Black), Bishop(Black),
                         Queen(Black), King(Black),
                         Bishop(Black), Knight(Black), Rook(Black)]
        self.field[1] = [Pawn(Black)] * 8
        self.field[-1] = [Rook(White), Knight(White), Bishop(White),
...
class Figure(Battlefield):
    ...
    def __repr__(self):
        if self.__class__ == Space:
            return ''
        elif self.__class__ == King:
            return '♔'
        else:
            return self.__class__.__name__[0]
    '''
    mode = 'common' - показывает, находится ли фигура под ударом и возвращает True/False
    mode = 'WhoAttack' - показывает, под чьим ударом находится фигура (возвращает объекты фигур,
    которые бьют данную фигуру. Если никакие фигуры не бьют данную, то вернёт пустой список)
    '''    
    def underAttack(self, battlefield, team, mode = 'common'): #team - под атакой какой команды 
        ...
        if mode == 'common':
            return Rook.show_moves(self, battlefield, 'check',team = team ) \
                ...
                or Bishop.show_moves(self, battlefield, mode = 'check', team = team)
        if mode == 'WhoAttack':
            attackers += Rook.show_moves(self, battlefield, 'WhoAttack', team)
            attackers += Pawn.show_moves(self, battlefield, 'WhoAttack', team)
            attackers += Bishop.show_moves(self, battlefield, mode = 'WhoAttack', team = team)
        ...
        for a, b in check:
            for i, j in zip(a, b):
                try:
                    ...
                    cell = show[x + i][y + j]
                    if cell.team == team and (cell.__class__ == Queen or cell.__class__ == Bishop):
                        if mode == 'check':
                            return True
                        if mode == 'WhoAttack':
                            attackers.append(cell)
...
class Queen(Figure):
    def show_moves(self, battlefield):
        ...
        return Bishop.show_moves(self, battlefield, 0, 'common', AMoves, AShow)
...
while True:
    try:
        ...
        if condition:
            raise Check
        print(condition)
    except Check: #Добавить, что можно закрыться
